-- Supabase Migration: Update provisioning trigger to set Mistral as default agent
-- Safe replacement of the create_default_tenant_for_new_user function
...
BEGIN
  ...
  -- 6. Default AI Agent (Updated to MISTRAL default)
  INSERT INTO public.ai_agents (tenant_id, name, model, instructions, temperature, is_active, metadata)
  VALUES (
    new_tenant_id,
    'Primary Support Agent',
    'mistral'::public.ai_provider,
    'You are a professional, helpful support assistant. Keep replies concise and friendly.',
    0.7,
    true,
    jsonb_build_object(
      'full_model', 'mistral-large-latest',
      'kbSources', '[]'::jsonb,
      'pipeline', 'Default Pipeline'
    )
  );

  -- 7. Default Workspace Settings Config
